...
@Composable
fun DashboardScreen(onNavigate: () -> Unit) {
    Scaffold(
        topBar = { SubsectionTopBar(title = "Reportes", onNavigate = onNavigate) }
    ) { innerPadding ->
        JetpackComposeBasicComboChart(modifier = Modifier.padding(innerPadding))
    }
}
